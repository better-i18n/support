...
import { realtimeStreams } from "@api/realtime/streams";
...
const visitorStreamHandler = handle({
        ...
        filter: async ({ event, request }) => {
                ...
                if (event.__event_path[0] !== "visitor") {
                        return false;
                }
                ...
                return !payloadVisitorId || payloadVisitorId === visitorId;
        },
});
...
const dashboardStreamHandler = handle({
        ...
        filter: ({ event }) => {
                return Array.isArray(event.__event_path) && event.__event_path[0] === "dashboard";
        },
